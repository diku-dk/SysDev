...
class Doctor:
    ...
    def check_available(self, clinic: Clinic, booking_date: date):
        ...
        if clinic.list_holidays.contains_day(booking_date):
            return f"The clinic is closed on {booking_date.strftime('%d/%m/%Y')} "
        elif self.list_holidays.contains_day(booking_date):
            return f"{self.name} is on holiday on {booking_date.strftime('%d/%m/%Y')}"
        else:
            return f"{self.name} is available on {booking_date.strftime('%d/%m/%Y')}"
